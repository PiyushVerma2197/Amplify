...
const REGEX = /.*-(\w+)/;
const AMPLIFY_ENV = amplifyMeta.storage.IdentityBrokerCodesTable.output.Name.match(REGEX)[1];

console.log("Injecting config");
console.log("AMPLIFY_ENV is " + AMPLIFY_ENV);

var appHostingDomain = undefined;
if (amplifyTeams[AMPLIFY_ENV].categories.function.idbAdminTenant.baseDn) {
    appHostingDomain = `https://auth.${amplifyTeams[AMPLIFY_ENV].categories.function.idbAdminTenant.baseDn}`;
...
if (appHostingDomain) {
    console.log("Using Hosting Domain: " + appHostingDomain)
